...
如使用 subagent 并行生成页面，必须遵守 `references/orchestration.md`：不得多个 agent 并发写同一个 `.pptx`；采用页面模块化装配模式，subagent 只产出 `pages/page_NN.py`，统一签名为 `def render(slide, ctx)`，主 agent 单进程按 storyline 顺序装配。
