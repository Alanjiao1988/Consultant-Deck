...
落盘动作：写入 `<private-state-root>/deck-drafts/<YYYY-MM-DD>/<deck-title-slug>/storyline.md`。
...
落盘动作：写入或更新 `<private-state-root>/deck-drafts/<YYYY-MM-DD>/<deck-title-slug>/evidence.json` 和 `assumptions.md`。所有进入 deck 的数字必须先登记到 `evidence.json`，再进入页面。
