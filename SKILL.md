...
落盘动作：生成开始前，在 GitHub repo 中建立 `Deck draft/<YYYY-MM-DD>/<deck-title-slug>/`，所有中间过程文件都写入该目录，不只保存在本地或对话中。
...
落盘动作：写入 `Deck draft/<YYYY-MM-DD>/<deck-title-slug>/storyline.md`。
...
落盘动作：写入 `Deck draft/<YYYY-MM-DD>/<deck-title-slug>/briefs.yaml`。
...
落盘动作：写入或更新 `Deck draft/<YYYY-MM-DD>/<deck-title-slug>/evidence.json` 和 `assumptions.md`。所有进入 deck 的数字必须先登记到 `evidence.json`，再进入页面。
...
落盘动作：确认通过或自动执行门通过后，冻结 `Deck draft/<YYYY-MM-DD>/<deck-title-slug>/baseline/` 基线快照。冻结 = 基线快照存在。
...
如使用 subagent 并行生成页面，必须遵守 `references/orchestration.md`：不得多个 agent 并发写同一个 `.pptx`；采用页面模块化装配模式，subagent 只产出 `Deck draft/<YYYY-MM-DD>/<deck-title-slug>/pages/page_NN.py`，统一签名为 `def render(slide, ctx)`，主 agent 单进程按 storyline 顺序装配。`ctx` 的最小 schema 由 `references/orchestration.md` 定义，页面 worker 不得自行发明新的必填字段。
...
```bash
python scripts/qa_pptx.py <deck.pptx> --facts Deck\ draft/<YYYY-MM-DD>/<deck-title-slug>/evidence.json
```
...
如果 `Deck draft/<YYYY-MM-DD>/<deck-title-slug>/baseline/` 已存在，任何变更请求都进入修订模式，不得默认整体重新生成。修订前先 diff 基线，列出目标页和牵连页；修订后按变更类型分级重跑 QA。详见 `references/revision-loop.md`。
...
6. 中间过程文件必须写入 GitHub repo 的 `Deck draft/` 目录，不得只保存在本地资源或对话历史中。
...
| `references/project-state.md` | GitHub repo 状态层与事实表 |
